...
const Carousel3 = () => {
  ...
  return (
    <>
      <CodeBox
        text={"Basic Slide Carousel with Autoplay"}
...
const Carousel = () => {
  ...
  return (
    ...
    <button onClick={prevSlide} className="absolute top-1/2 left-3 z-50 hover:bg-black/50 p-2 rounded-full w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white bg-opacity-20 text-white" aria-label="Previous Slide">
      {/* Previous slide icon */}
       <svg className="h-6 w-6" viewBox="0 0 24 24" stroke="currentColor" fill="none">
         <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
       </svg>
    </button>
    <button onClick={nextSlide} className="absolute top-1/2 right-3 z-50 hover:bg-black/50 p-2 rounded-full w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white bg-opacity-20 text-white" aria-label="Next Slide">
      {/* Next slide icon */}
       <svg className="h-6 w-6" viewBox="0 0 24 24" stroke="currentColor" fill="none">
    ...
    <div className="overflow-hidden w-full" style={{ aspectRatio: "23/11" }}>
      <div className="flex w-full transition-transform duration-300" style={{ transform: \`translateX(-\${currentIndex * 100}%)\` }}>
        {items.map((slide, i) => (
          <img
            key={i}
            src={slide}
